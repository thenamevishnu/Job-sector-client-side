...
function ChangePassword() {
    ...
    const submitHandle = async (e) => {
        e.preventDefault()
        const response = await handleChangePassword(id, password)
        if(response){
            dispatch(updateUser({}))
    ...
    return (
        <div className="relative flex justify-center mt-20 overflow-hidden">
            <div className="w-full p-6 m-auto bg-white rounded-2xl shadow-2xl border-2 md:max-w-xl">
                ...
                <div className="mb-2">
                        <input type='password' className='block w-full px-4 py-2 mt-2 text-green-700 bg-white border border-green-400 rounded-md focus:border-green-400 focus:ring-green-300 focus:outline-none' placeholder='Enter Current password' name='oldPassword' value={password.oldPassword} onChange={(e)=>setPassword({...password,[e.target.name]:e.target.value})}></input>
                    </div>
                    <div className="mb-2">
